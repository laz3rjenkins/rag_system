...
# # SECTION_PATTERN = r'(?=^\d+(?:\.\d+)*\s*(?:[А-ЯA-ZЁ].+)?)'
# SECTION_PATTERN = r'(?=^\d+(?:\.\d+)*\s*[А-ЯA-ZЁ]?.*)'
# APPENDIX_PATTERN = r"(?=^Приложение\s+[А-ЯA-ZА-ЯЁ])"
#
#
# def split_gost_sections(text: str):
#     pattern = f"{SECTION_PATTERN}|{APPENDIX_PATTERN}"
#
#     sections = re.split(pattern, text, flags=re.MULTILINE)
#
#     return [s.strip() for s in sections if len(s.strip()) > 15] # 80
TOP_SECTION_PATTERN = r'(?=^\d+\s+[А-ЯA-ZЁ])'

def split_top_sections(text: str):
    # разделяем только по верхним разделам
